...
public class CaptureTheFlagGameType extends AbstractTeamGameType {
	...
	class FlagOnTouchListener implements OnTouchListener {
		...
		public void onTouch(Entity me, Entity other) {
			/*
			 * Refactoring target : if else statement Refactoring name :
			 * refactoring deeply nested block statement Bad smell(reason) :
			 * nested block
			 */
			...
			/*
			 * Now, based on which team the player is on, we determine what to
			 * do with the flag.
			 * 
			 * If the player is on an opposing team, they force the flag back to
			 * their home base
			 */
			int teamId = otherPlayer.getTeam().getId();
			boolean typeCheck = (teamId == Team.ALLIED_TEAM_ID
					&& flag.getType() == seventh.game.entities.Entity.Type.ALLIED_FLAG)
		...
		/*
		 * Refactoring target : else statement Refactoring name : extract method
	...
	private Vector2f alliedFlagSpawn, axisFlagSpawn;
	 ...
	 * @param matchTime
	 */
	public CaptureTheFlagGameType(Leola runtime, List<Vector2f> alliedSpawnPoints, List<Vector2f> axisSpawnPoints,
			int maxScore, long matchTime, Vector2f alliedFlagSpawn, Vector2f axisFlagSpawn, Rectangle alliedHomeBase,
			Rectangle axisHomeBase, long spawnDelay) {
	...
	protected GameState doUpdate(Game game, TimeStep timeStep) {
		...
		if (GameState.IN_PROGRESS == getGameState()) {
			...
			/*
			 * Check and see if this flag has been captured
			 */
			if (this.axisFlag.isBeingCarried()) {
				if (this.axisFlag.getBounds().intersects(axisHomeBase)) {
					getDispatcher().queueEvent(
							new FlagCapturedEvent(this, this.axisFlag, this.axisFlag.getCarriedBy().getId()));
			...
			if (this.alliedFlag.isBeingCarried()) {
				if (this.alliedFlag.getBounds().intersects(alliedHomeBase)) {
					getDispatcher().queueEvent(
							new FlagCapturedEvent(this, this.alliedFlag, this.alliedFlag.getCarriedBy().getId()));
